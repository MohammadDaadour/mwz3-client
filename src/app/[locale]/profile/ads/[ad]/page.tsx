...
export default function UserAdsItem({ params: { locale, ad } }: Props) {
  ...
  if (context.user.id > 0 && data && area && images) {
    return (
        ...
        <Flex direction='column' p='xs' gap='xs' pos='relative' classNames={{ root: classes.mainSection }}>
          <Group justify='space-between' align='start' wrap='nowrap'>
            <Stack>
              ...
              <Text>{`${t("lblNote")} ${data.notes ? data.notes : ""}`}</Text>
            </Stack>
            <Button disabled={data.stateFK === 6} component={Link} href={`/profile/ads/${data.id.toString(16)}/edit`}>
              {t("btnEdit")}
            </Button>
          </Group>
          <Box>
